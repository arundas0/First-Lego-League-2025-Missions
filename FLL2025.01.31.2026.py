...
def mission_0():
   ...
   # wait(200)
    ...
    # gyro_turn(-12)
    #wait(200)
    gyro_turn(-25,settle_timeout_ms=1500)
    #wait(200)

    drive_cm(44, 50, 50)
    wait(200)
    #gyro_turn(55, mode="slow")
    gyro_turn(66,mode="slow")
   
    wait(200)
    drive_cm(15, 30, 20) # Changed here from 14 to 15
...
def mission_5():
    ...
    drive_cm(19.5, 50, 30)
    robot.turn(10)
    motor_c.run_until_stalled(-200, then=Stop.BRAKE, duty_limit=50) #drop gear mechanism
    drive_cm(2, 30, 10)
    ...
    # Wait for either stall or timeout (2000ms = 2 seconds)
    while timer.time() < 3000:
        if motor_d.stalled():
            break
        wait(10)  # Small delay to avoid busy-waiting

    # Stop the motor with brake
    motor_d.brake()
    ...
    drive_cm(-58, 30, 50) #back to home
 
def mission_6(): #Deposit Stuff
    setup_drive(0)
    drive_cm(50, 50, 25) #drop stuff in deposit zone
    drive_cm(-50, 100, 100) #back to home
...
def mission_7(): #Raise Hell
    ...
    drive_cm(20, 30, 50)
    run_motor_for_degrees(motor_d, 250, 100)
    #run_motor_for_degrees(motor_d, 300, 500) #raise arm after hitting fossil
    gyro_turn(-30, mode="slow")
    drive_cm(-25, 100, 50)
    run_motor_for_degrees(motor_d, 500, 500)
    gyro_turn(-92, mode="fast")
    drive_cm(-70, 40, 100)

    print("Total motor run time,mission7:", watch.time(), "ms")

def mission_8(): #Raise Hell
    setup_drive()
   
    hub.imu.reset_heading(0)
    wait(100)

    drive_cm(65, 100, 50) #move forward to wall //79
    # gyro_turn(90)
    # motor_c.run_until_stalled(600, then=Stop.BRAKE, duty_limit=35) #lower arm trolley
    # drive_cm(14, 30, 50) #13
    # run_motor_for_degrees(motor_c, -400, 300) #raise arm trolley
